...
from .inspect_shelf import load_shelf
...
parser.add_argument('files', type=str, nargs='+', help='shelf store file')

args = parser.parse_args()

# fuse all files (overriding older)
data = load_shelf(args.files[0])
for file in args.files[1:]:
    for k, v in load_shelf(file).items():
        if v:
            data[k] = v
